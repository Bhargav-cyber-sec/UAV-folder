...
| 13 | Wheelhouse built with the wrong running Python silently omits conditional dependencies | `--python-version 314` only controls which wheel tags pip fetches; it does NOT control evaluation of conditional deps like `exceptiongroup; python_version < "3.15"` — that's evaluated against whichever Python actually runs pip. Discovered when `pip install --no-index -r requirements.txt` failed on a missing `exceptiongroup` (a transitive dep of anyio/httpx/huggingface-hub/faster-whisper) because the wheelhouse was built by `python` resolving to 3.15, not 3.14. Fixed by calling `py -3.14 -m pip download` explicitly in `build_wheelhouse.bat`, not plain `python` | `build_wheelhouse.bat` (uses `py -3.14` explicitly) | 1, ongoing |cl