...
REM
REM IMPORTANT: this script calls "py -3.14 -m pip download", NOT plain
REM "python -m pip download". --python-version 314 alone only controls which
REM wheel TAGS pip looks for; it does NOT control how pip evaluates
REM conditional dependencies like "exceptiongroup; python_version < '3.15'" -
...
REM quietly missing from the wheelhouse. Running via "py -3.14" fixes this
...
py -3.14 -m pip download torch torchvision ^
...
py -3.14 -m pip download ultralytics --no-deps ^
    -d %OUTDIR% ^
    --python-version %PYVER% ^
    --platform %PLATFORM% ^
    --only-binary=:all:
...
py -3.14 -m pip download -r requirements.txt ^
    -d %OUTDIR% ^
    --python-version %PYVER% ^
    --platform %PLATFORM% ^
    --only-binary=:all:
